...
def inverse_dft(dft_in, wnd_title='filtered_img', normalize=False):
    ...
    cv2.imshow(wnd_title, filtered_img)


def modify_dft(dft, x, y, modify=False):
    ...
    # Show base image
    dft_base = np.zeros_like(dft_shift)
    dft_base[y, x, 1] = np.sign(dft_shift[y, x, 1]) * max_coef
    if y != h - y and x != w - x:  # if not the center position
        # Assure complex conjugate symmetry
        dft_base[h - y, w - x, 1] = -1.0 * dft_shift[y, x, 1]

    inverse_dft(dft_base, 'dft base', True)


def on_mouse_click(event, x, y, flags, param):
    global button_pressed, dft_shift

    if event == cv2.EVENT_LBUTTONDOWN:
        button_pressed = True
        modify_dft(dft_shift, x, y)

    if event == cv2.EVENT_LBUTTONUP:
        button_pressed = False

    if event == cv2.EVENT_RBUTTONDOWN:
        modify_dft(dft_shift, x, y, True)

    if event == cv2.EVENT_MOUSEMOVE and button_pressed:
        modify_dft(dft_shift, x, y)
...
cv2.setMouseCallback('dft log magnitude', on_mouse_click)
